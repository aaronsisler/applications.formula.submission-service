import PDFDocument from "pdfkit";

import { SideBySideLabelAndValue } from "../../components/side-by-side-label-and-value";
import { ApplicationMarkupField } from "../../models/application-markup-field";
import { InputFieldName } from "../../models/input-field-name";
import { buildHeader } from "../../utils/pdf-utils";

export const ApplicantTransportationDisplayGroup = (
  pdfDocument: typeof PDFDocument,
  applicationInputFields: Map<string, any>
): typeof PDFDocument => {
  // TODO Make this bold and import font types
  // http://pdfkit.org/docs/text.html
  pdfDocument = buildHeader(
    pdfDocument,
    "Applicant Transportation Information"
  );
  pdfDocument.moveDown(0.75);
  let currentField: ApplicationMarkupField;

  currentField = applicationInputFields.get(
    InputFieldName.TRANSPORTATION_DRIVERS_LICENSE_NUMBER
  );
  pdfDocument = SideBySideLabelAndValue(pdfDocument, currentField, 25, true);

  currentField = applicationInputFields.get(
    InputFieldName.TRANSPORTATION_DRIVERS_LICENSE_STATE
  );
  pdfDocument = SideBySideLabelAndValue(pdfDocument, currentField, 5);

  currentField = applicationInputFields.get(
    InputFieldName.TRANSPORTATION_DRIVERS_LICENSE_VALID
  );
  pdfDocument = SideBySideLabelAndValue(pdfDocument, currentField, 5);

  currentField = applicationInputFields.get(
    InputFieldName.TRANSPORTATION_DRIVERS_LICENSE_VALID_NO
  );
  pdfDocument = SideBySideLabelAndValue(pdfDocument, currentField, 100);

  return pdfDocument;
};
